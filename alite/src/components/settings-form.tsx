...
export default function SettingsForm({ initialName, initialCurrency, email }: SettingsFormProps) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await updateProfile({ full_name: fullName, base_currency: baseCurrency })
      if (result.error) {
  ...
  return (
    <form onSubmit={handleSave} className="space-y-3">
      ...
      <div className="bg-card border border-border rounded-2xl px-4 py-4 space-y-4">
        ...
        <div>
          <label className="text-[10px] font-medium text-muted-foreground block mb-1.5">Full name</label>
          <input
            type="text"
            value={fullName}
        ...
        <div className="border-t border-border pt-4">
          <label className="text-[10px] font-medium text-muted-foreground block mb-1.5">Email</label>
