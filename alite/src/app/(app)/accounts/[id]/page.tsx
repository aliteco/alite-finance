      ...
      <div className="max-w-lg mx-auto px-4 pt-8 space-y-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <Link
            ...
            className="text-xs font-medium text-muted-foreground hover:text-foreground transition-colors px-3 py-1.5 rounded-lg border"
        ...
        >
          ...
          {account.color && (
            <div
              ...
              style={{
                background: `radial-gradient(ellipse 70% 50% at 90% -10%, ${account.color}18 0%, transparent 70%)`,
              }}
            />
          )}
          ...
          <div className="flex items-start gap-3.5">
            ...
            <div
              ...
              style={{
                background: account.color ? `${account.color}22` : 'rgba(255,255,255,0.06)',
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                ...
                {!account.include_in_net_worth && (
                  <span className="ml-1.5 opacity-50">· excluded from net worth</span>
                )}
              </p>
            </div>
          </div>
          ...
          >
            <div>
              ...
              <p className="text-sm font-semibold tabular-nums text-income">
                +{formatCurrency(monthlyIn, account.currency)}
              </p>
            </div>
            <div>
              <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-1">
              ...
              <p className="text-sm font-semibold tabular-nums text-expense">
                −{formatCurrency(monthlyOut, account.currency)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Link
            ...
            className="flex items-center justify-center gap-2 h-10 rounded-xl text-xs font-semibold text-expense transition-colors hover:opacity-80"
            style={{
          ...
          <Link
            ...
            className="flex items-center justify-center gap-2 h-10 rounded-xl text-xs font-semibold text-income transition-colors hover:opacity-80"
            style={{
        ...
        <section>
          ...
          {transactions.length === 0 ? (
            ...
            >
              ...
              <Link
                href={`/transactions/new?account=${id}`}
...
function TxRow({
  ...
  const dotColor = isIncome ? '#34d399' : isTransfer ? '#818cf8' : '#f87171'
  const sign = isIncome ? '+' : '−'
  ...
  return (
    <div
      ...
      style={
        hasBorder ? { borderBottom: '0.5px solid rgba(255,255,255,0.05)' } : {}
      }
    >
      {/* Category dot / icon */}
      <div
        ...
        style={{
          background: category?.color ? `${category.color}22` : dotBg,
          color: category?.color ?? dotColor,
        }}
      >
        {isTransfer
          ? '⇄'
          : (category?.name ?? label).charAt(0).toUpperCase()}
      </div>
      ...
      <div className="text-right shrink-0">
        ...
        {tx.currency !== baseCurrency && (
          <p className="text-[10px] text-muted-foreground tabular-nums">
            ...
            }).format(tx.base_currency_amount)}
          </p>
        )}
      </div>
    </div>
